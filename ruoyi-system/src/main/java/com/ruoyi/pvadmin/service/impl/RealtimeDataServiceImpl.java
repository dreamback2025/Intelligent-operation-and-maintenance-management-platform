    ...
    private Map<String, TagValue> getTagValues(List<String> deviceCodes, List<DeviceIndex> indexList) {
        ...
        if (CollectionUtils.isNotEmpty(indexList)) {
            if (!deviceCodes.isEmpty() && !deviceCodes.contains(null)) {
                // 使用完整的点位编号（如 INV_DEMO_01_PW）而非设备编号，
                // 确保 mock 回退时生成的 TagValue.tagCode 与后续 map 查询键一致
                List<String> indexCodes = indexList.stream()
                        .map(DeviceIndex::getIndexCode)
                ...
                List<TagValue> deviceTagValueList = realtimeDatabaseService.retrieve(indexCodes);
